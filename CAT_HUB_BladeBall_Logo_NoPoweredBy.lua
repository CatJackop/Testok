...
██║ ██║ ██║ 
╚═╝ ╚═╝ ╚═╝ 
CAT HUB - BLADE BALL ULTIMATE 
TG: CAT HUB 
DC: https://discord.gg/KckKFJWzs6 
Credit: catjack.gg 
]] 
...
ScreenGui.Name = "CATHUB_HideShow"

local function CreateCATLogoButton()
    local LogoButton = Instance.new("ImageButton")
    LogoButton.Name = "CATHUBLogo"
    LogoButton.Parent = ScreenGui
    LogoButton.Size = UDim2.new(0, 58, 0, 58)
    LogoButton.Position = UDim2.new(0, 18, 0.5, -29)
    LogoButton.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
    LogoButton.BackgroundTransparency = 0.15
    ...
    local function setPanelsVisible(state)
        visible = state
        if MainFrame then MainFrame.Visible = state end
        if TBFrame then TBFrame.Visible = state end
        if statsFrame then statsFrame.Visible = state and cfg.showStats end
        if BindFrame and not state then BindFrame.Visible = false end
    end
...
local CATLogoButton = CreateCATLogoButton()
...
local Win = NEVERLOSE:AddWindow("CAT HUB", " ") 
...
SocialSec:AddLabel('TG: CAT HUB') 
SocialSec:AddLabel('DC: https://discord.gg/KckKFJWzs6') 
SocialSec:AddLabel('Credit: catjack.gg') 
SocialSec:AddButton('Copy Discord', function() 
setclipboard("https://discord.gg/KckKFJWzs6") 
end) 
end
