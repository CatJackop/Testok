...
local CAT_HUB_ASSET_ID = "126031329785796"
...
iData.value10.SHAPE_FILE = "CATHUBPlaceShape.txt"
...
handler(function()
	...
	while iData.value17() do
		...
		if iData.value14.AutoSteal and iData.value23() then
			...
			if not success then
				warn("[CATHUB] steal: " .. tostring(secondaryResult))
				task.wait(0.5)
			end
		else
...
function iData.value90()
	...
	iData.value11.espFolder.Name = "CAT HUBEggESP"
...
iData.value10.HOP_FILE = "CATHUBHopCount.txt"
function iData.value113(shardHops)
	...
	if getgenv then
		getgenv().__CATHUBHops = shardHops
	end
end
iData.value14.HopCount = (function()
	...
	return tonumber(getgenv and getgenv().__CATHUBHops) or 0
end)()
...
function iData.value138()
	...
	ScreenGui.Name = "CATHUBStealFreeGui"
	ScreenGui.ResetOnSpawn = false
	ScreenGui.IgnoreGuiInset = true
...
local function createServerHopGui()
	...
	TitleLabel.Text = "CATHUB Hop Sever"
	...
	CreateTextRow("• Made By catjack.gg")
...
local function createStealGui()
	...
	local ScreenGui = Instance.new("ScreenGui")
	ScreenGui.Name = "CATHUBRecallStealGui"
	ScreenGui.ResetOnSpawn = false
	ScreenGui.IgnoreGuiInset = true
...
iData.value12.TabConfig:Button({
	...
	Callback = function()
		...
		if writefile then
			pcall(writefile, "CATHUBRemotes.txt", concatResult)
		end
		...
		task.spawn(function()
			...
			repeat
				if copyPlotDumpFlag or not nameOption then
					...
					if writefile then
						pcall(writefile, "CATHUBPlot.txt", concatResult)
					end
					iData.value18("Plot Dump", "Copied. Also saved as CATHUBPlot.txt.", 6)
	...
	Callback = function()
		task.spawn(function()
					...
					if descendant:IsA("ModuleScript") or descendant:IsA("LocalScript") then
						...
						if lower:find("egg") or (lower:find("plac") or (lower:find("build") or lower:find("plot"))) then
							...
							then
								...
								pcall(writefile, "CATHUBPlace_" .. descendant.Name .. ".txt", copyPlaceSourceResult)
			...
			if secondaryDescendant and copyPlaceSourceFlag then
				...
				if success then
					pcall(writefile, "CATHUBEggState.txt", copyPlaceSourceResult)
			...
			if writefile then
				pcall(writefile, "CATHUBPlaceSrc.txt", concatResult)
			end

			iData.value18("Place Source", "Copied. Saved CATHUBPlaceSrc.txt + per-module files.", 6)
		end)
	end,
})
iData.value12.TabConfig:Button({
...
iData.value143.Name = "CATHUBVisualPets"
...
if not iData.value24 then
	iData.value18("CATHUB", "Packages.Networking is missing, remote features are offline.", 8)
	return
end
iData.value18("CATHUB", "Loaded. " .. #iData.value13.areaOrder .. " areas, " .. #iData.value28() .. " endpoints.", 5)